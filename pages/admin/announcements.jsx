...
export default function AdminAnnouncements() {
  ...
  const [form, setForm] = useState({ title: '', body: '', visible_to: 'members' });
  ...
  const save = async (e) => {
    ...
    setBusy(false);
    if (r.ok) {
      setForm({ title: '', body: '', visible_to: 'members' });
      load();
    } else {
      alert('Save failed');
    }
  ...
  return (
    <Layout title="Admin • Announcements">
      <AdminOnly>
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-xs text-gray-500">
                          {new Date(a.created_at).toLocaleString()} • {a.visible_to}
                        </div>
                      </div>
