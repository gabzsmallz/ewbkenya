...
export default function AdminAnnouncements(){
  ...
  const load=async()=>{ const r=await fetch('/api/admin/announcements'); setItems(await r.json()||[]); }; useEffect(()=>{load();},[]);
  const save=async(e)=>{ e.preventDefault(); setBusy(true);
    ...
    setBusy(false); if(r.ok){ setForm({title:'',body:'',visible_to:'members'}); load(); } else alert('Save failed');
  };
  const del=async(id)=>{ if(!confirm('Delete announcement?')) return;
    const r=await fetch('/api/admin/announcements',{method:'DELETE',headers:{'Content-Type':'application/json'},body:JSON.stringify({id})});
  ...
  return(<Layout title="Admin • Announcements"><AdminOnly>
    <div className="grid md:grid-cols-2 gap-6">
      ...
      <div className="card"><h2 className="text-xl font-semibold mb-3">Announcements</h2>
        <div className="space-y-3">{items.map(a=>(<div key={a.id} className="border rounded p-3">
          <div className="flex items-center justify-between"><div><div className="font-semibold">{a.title}</div><div className="text-xs text-gray-500">{new Date(a.created_at).toLocaleString()} • {a.visible_to}</div></div>
          <button className="btn" onClick={()=>del(a.id)}>Delete</button></div></div>))}{items.length===0&&<p>No announcements yet.</p>}</div>
      </div>
    </div>
  </AdminOnly></Layout>);
}
