...
export default function AdminProjects(){
  ...
  useEffect(()=>{ load(); },[]);
  const save=async(e)=>{ e.preventDefault(); setBusy(true); const r=await fetch('/api/admin/projects',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(form)}); setBusy(false); if(r.ok){ setForm({title:'',slug:'',summary:'',description:'',status:'planned',cover_image_url:''}); load(); } else alert('Save failed'); };
