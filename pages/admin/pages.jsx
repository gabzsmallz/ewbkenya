import dynamic from 'next/dynamic'; import Layout from '../../components/Layout'; import AdminOnly from '../../components/AdminOnly'; import { useEffect, useState } from 'react'; import UploadImage from '../../components/UploadImage';
...
export default function AdminPages(){
  ...
  const load=async()=>{ const r=await fetch('/api/admin/pages?slug=home'); const d=await r.json(); if(d) setPage(p=>({...p,...d})); };
  useEffect(()=>{load();},[]);
  const save=async(e)=>{ e.preventDefault(); setBusy(true);
    const r=await fetch('/api/admin/pages',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(page)});
    setBusy(false); if(!r.ok) alert('Save failed');
  };
  return(<Layout title="Admin • Pages"><AdminOnly>
    <div className="card"><h2 className="text-xl font-semibold mb-3">Landing Page Content</h2>
