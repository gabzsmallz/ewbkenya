...
export default function Project({project,updates}){
  ...
  return(<Layout title={project.title}>
    <div className="card shadow-brand"><div className="flex items-center justify-between"><h1 className="text-2xl font-bold" style={{color:'var(--brand-primary)'}}>{project.title}</h1><span className="badge">{project.status}</span></div>{project.cover_image_url&&(
      <img
        src={project.cover_image_url}
        alt={`${project.title} cover image`}
        className="rounded mt-3 w-full max-h-96 object-cover"
      />
    )}<p className="mt-3">{project.description}</p></div>
