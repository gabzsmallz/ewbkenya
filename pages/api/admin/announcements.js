...
export default async function handler(req,res){
  ...
  if(req.method==='POST'){ const body=req.body||{}; const {error}=await supabaseAdmin.from('announcements').insert(body); if(error) return res.status(500).json({error:error.message}); return res.status(200).json({ok:true}); }
