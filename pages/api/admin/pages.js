...
export default async function handler(req,res){
  ...
  if(req.method==='POST'){ const b=req.body||{}; const row={ slug:b.slug||'home', title:b.title, content:b.content, hero_image_url:b.hero_image_url };
    const { error } = await supabaseAdmin.from('pages').upsert(row,{onConflict:'slug'}); if(error) return res.status(500).json({error:error.message}); return res.status(200).json({ok:true}); }
